...
type cfg struct {
	...
	CleanDir bool              `json:"cleanDir,omitempy"` // 是否清空目录
	...
	Model    []*cfgModel       `json:"model,omitempy"`    // 用于生成list的结构代码
}
...
type cfgModel struct {
	...
	Field [][3]string `json:"field,omitempy"` //
}

func (c *cfgModel) Check() {
	for i, f := range c.Field {
		if f[0] == "" {
			panic(fmt.Errorf("model[%d].field[0] is empty", i))
		}
		if f[1] == "" {
			c.Field[i][1] = "string"
		}
...
func genCode(config string) {
	...
	case db2go.MYSQL:
		...
		for _, m := range c.Model {
			_, err = code.ModelTPL(m.Name, m.Field)
			log.CheckError(err)
		}
		// 保存
