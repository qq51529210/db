...
var (
	_execTPL = template.Must(template.New("execTPL").Parse(`
{{- if .Sql}}
// {{.Sql}}
{{end -}}
...
`))
	_queryTPL      = template.Must(template.New("queryTPL").Parse(``))
	_execStructTPL = template.Must(template.New("execStructTPL").Parse(`
type {{.Struct}} struct {
	{{- range .Field}}
...
`))
	_queryStructTPL = template.Must(template.New("queryStructTPL").Parse(``))
	_fileTPL        = template.Must(template.New("fileTPL").Parse(`package {{.Pkg}}
